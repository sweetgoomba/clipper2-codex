...
최신 단계: 실제 개발서버 전환은 아직 실행하지 않았고, [개발서버 정식 PG 전환 실행 런북](../implementation/2026-09-18-development-pg-cutover-runbook.md)을 검토용 초안으로 작성했다. m2-stage 사전 image/env 보존과 정확한 source SHA 고정 → Web/Admin/API 사전 build → 서비스 쓰기 정지 → m2-db의 최종 User/Admin/Release dump·off-host 복사·보존 hash → User→Admin→Release migration → no-op·hash·정책 검증 → 새 서비스/새 개발판 smoke → 세 DB dump+세 image 일괄 rollback을 Gate A–G로 나눴다. 이 문서 작성은 실행 승인이 아니며 서버 접속·실제 DB 변경·배포는 0건이다. Web API `fe58b65`, Angular `19b407a7`, Nest `884fa8bc`와 나머지 Web/Admin/Infra/Client는 원격 `integration/dev-pg-local-validation-20260917`에 반영돼 로컬 HEAD와 일치한다. push 직전 fresh 검증은 Angular 전체 4,494 + 스타일 6 PASS와 `build:devapp` PASS, Nest 전체 2,685 PASS와 build PASS다. Nest 전체 테스트는 실제 `.env.local`의 JWT 설정과 테스트 서버를 격리하기 위해 `CLIPPER_AUTH_MODE=local`을 명시했다. Infra 배포·PG env 자동검증 95 tests도 PASS다. 다음은 런북 내용 확인과 실제 전환 날짜·60분 점검 창 결정이며, Gate A 실행은 별도 확인한다.
