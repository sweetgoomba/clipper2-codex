...
최신 W04 비-ML 로컬 acceptance와 개발 DB 복제본 리허설 완료: Desktop Nest 115+41+37 및 후속 outbox/restart 27 PASS, Angular 202 및 후속 recovery/account 24 PASS, Web API·Electron·Web/Admin/Infra 자동 검증 통과. 사용자가 m2-db에서 만든 실제 dev DB 3개 read-only dump의 크기·SHA를 로컬에서 재확인하고 새 59433–59435 전용 PostgreSQL 16 clone에 복원했다. User/Release/Admin migration, 재실행 no-op, 핵심 건수·ID 해시 보존, 별도 rollback-check DB의 dump 복원, clone API `/health` 3DB ok를 확인했다. 기존 사용자20명·session162·project1481과 운영자/provider/release 데이터는 보존됐고, 옛 license24·credit ledger253·operation run210·review payment는 승인된 출시 전 정책대로 제거되어 신규 PG 금융 테이블은 0행이다. API 기동 전후 기존 사용자 무료체험 소급 지급도 0건이었다. 실제 개발 DB/서비스 변경·배포는 없다. 새로 `/catalog`에서 `entitlement_mode=all`인데 과거 `pluginKeys`가 Basic5/Pro6/Business0으로 노출되는 의미상 불일치를 발견했다. 실제 operation 시작은 allowlist를 사용하지 않아 기능 제한은 없지만 계약 정리가 필요하므로 사용자 승인 전 개발서버 전환 계획 확정은 멈춘다. [복제본 상세 결과](../implementation/2026-09-18-development-db-clone-rehearsal-result.md)와 [전체 acceptance](../implementation/2026-09-18-w04-non-ml-local-acceptance.md)를 따른다. 실제 ML/Build5, 에이전트의 서버 접속·배포는 HOLD다.
