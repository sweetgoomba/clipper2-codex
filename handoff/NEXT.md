...
준비·최종 렌더까지 실제 provider E2E를 완료했다. 운영 프로필의 여러 목적을 최대 2개의
독립된 YouTube 조사 범위로 나누는 보완까지 2026-07-31에 구현·검증·push했다.

다음 목표는 현재 AI 디렉터에서 `운영 프로필 → 조사 → 조사 주제 → 영상 후보 → 구체적인
스토리보드`까지를 별도 **숏폼 스토리보드 생성기 플러그인**으로 분리하는 것이다. 실제
미디어 생성·소재 준비·최종 영상 렌더는 새 플러그인의 경계 밖으로 둔다. 다만 정확히 어느
artifact까지를 완성된 스토리보드로 볼지는 기존 코드를 먼저 조사한 뒤 설계 단계에서
확정한다.

새 플러그인 분리와 함께 사용자용 UI/UX 정리, 디버그 JSON 분리, provider 예외 처리,
호출 후 실제 비용 집계, 장시간 작업을 위한 플러그인 내부 영속 큐와 완료 알림을 차례로
설계·구현한다. 범위가 큰 작업이므로 한 번에 구현하지 않고 독립된 설계·계획·검증 단위로
나눈다.
...
1. 아래 저장소 기준점과 branch/status/upstream/recent log를 확인한다.
2. 현재 AI 디렉터의 실제 코드와 JSON artifact 흐름을 따라 다음 경계를 도식화한다.
   - 운영 프로필
   - discovery와 reference analysis
   - 조사 주제
   - 영상 후보
   - 상세 영상 계획·장면별 미디어 결정
   - 소재 준비·미디어 생성
   - 최종 렌더·보관함 큐
3. 새 스토리보드 플러그인이 어디까지 소유하고 무엇을 기존 AI 디렉터와 공유할지
   2~3가지 분리안을 제시한다.
4. 사용자 승인 전에는 새 브랜치 생성이나 코드 수정을 시작하지 않는다.
5. 승인 뒤 관련 저장소의 현재 브랜치에서 동일한 새 feature branch를 만든다. worktree는
   만들지 않는다.
6. 이후 작업은 다음처럼 별도 설계·계획·구현 커밋으로 나눈다.
   - 플러그인 경계와 데이터 계약
   - 사용자용 UI/UX와 디버그 화면 분리
   - provider 오류 처리·재시도·부분 성공·재개
   - 예상 비용과 실제 사용 비용의 단계별·전체 흐름 집계
   - 플러그인 내부 영속 큐·진행 상태·완료 알림

## 다음 플러그인의 필수 방향

- 기존 AI 디렉터의 실제 provider 구현과 로컬 JSON 저장소를 재사용한다.
- API 키는 기존 관리자 페이지 → Web API credential 구조만 사용한다.
- 기존 AI 디렉터와 새 플러그인 모두 크레딧을 차감하지 않는다.
- 사용자 화면은 사람이 읽는 요약과 다음 행동을 먼저 보여준다.
- raw JSON, artifact ID, provider 응답, 상세 디버그 정보는 사용자 기본 화면에서 분리해
  접힌 고급 정보 또는 개발자 디버그 화면에서만 보여준다.
- provider 호출마다 timeout, 취소, 오류 분류, 안전한 재시도, 부분 성공, 중복 실행 방지를
  현재 구현과 비교해 빠진 부분부터 보완한다.
- 비용은 실행 전 예상 비용과 실행 후 실제 provider usage 기반 비용을 구분한다. 실제
  usage를 제공하지 않는 provider는 추정값임을 명시한다.
- 비용은 호출별·단계별·조사 실행별·스토리보드 전체 흐름별로 확인할 수 있어야 한다.
- 장시간 작업은 화면을 붙잡아두지 않고 영속 큐에 넣는다. 앱을 다시 열어도 상태를
  복원하고 완료·실패 알림, 재시도, 취소 가능 여부를 설계한다.
- 기존 전역 보관함 영상 렌더 큐와 새 스토리보드 작업 큐의 책임을 섞지 않는다.
- 더미 응답이나 fixture로 완료를 주장하지 않는다. 실제 provider E2E는 비용 사전 점검과
  사용자 승인 뒤에만 실행한다.
- 기존 AI 디렉터의 전체 렌더 경로는 새 플러그인 분리 과정에서 깨뜨리지 않는다.
...
| `desktop/clipper_python` | `dev` | `600a231` | origin과 동기화 |
