...
## 비용 기준선 (2026-08-26 확인)

- 할인된 Luna: 입력 `$0.20`, 출력 `$1.20`/100만 토큰
- Nano: 입력 `$0.20`, 출력 `$1.25`/100만 토큰
- 보고용 환율: `$1=1,400원`
- 주제 뽑기: 약 15~16원
- 영상 후보: 약 8~11원
- 스토리보드: 약 13~16원
...
이는 로컬 artifact에 저장된 usage 기반 계산값이며, cached input 미보존과 실패·재시도 호출은
별도 비용이 될 수 있다. 상세 표본과 계산식은
`.codex/records/sessions/2026/08/25.md`의 `실제 비용 기준선` 절을 본다.
