...
1. 명령·환경 파일·저장소 존재·미커밋 변경을 확인한다. 개발 dev / 운영 main 같은 소스 브랜치명 제한은 없다. 다른 브랜치를 자동 checkout하거나 파일을 삭제/stash하지 않는다.
2. 현재 체크아웃한 infra 브랜치에서 `git pull --ff-only`로 연결된 원격 브랜치를 기준으로 갱신한다. 실행부가 갱신되면 새 실행부로 재실행한다.
3. 해당 환경의 Compose 설정을 읽고 로컬 이미지 이름을 검증한다. API 선택 시 DB 대상과 PG 설정도 확인한다. 설정값 전체나 비밀은 출력하지 않는다.
4. 선택한 소스 저장소 각각에서도 현재 체크아웃한 브랜치에서 `git pull --ff-only`로 갱신하고, 그 서버에서 Docker 이미지를 하나씩 빌드한다. 모두 성공한 다음 선택한 컨테이너만 다시 실행한다. 다른 회사 프로젝트는 대상에 넣지 않는다.
...
소스 브랜치와 배포 환경은 별개다. 예를 들어 운영 서버의 저장소가 `dev`를 체크아웃했더라도 `deploy-prod.sh web`은 그 소스를 운영 설정(`ANGULAR_CONFIGURATION=prod`, 운영 API 주소)으로 빌드한다. 각 저장소의 브랜치를 동일하게 강제하지도 않는다. 배포할 코드는 실행 전에 사용자가 선택한다. 연결된 원격 브랜치가 없거나 fast-forward할 수 없으면 Git 갱신 실패로 중단하며 임의로 origin/dev 또는 origin/main을 선택하지 않는다. `--start-only`와 migration은 기존대로 pull하지 않는다.
