...
데스크톱 운영판 이름·아이콘은 별도 결정 대기다. [앱 이름·환경 분리 결정 대기 기록](./2026-09-07-desktop-dev-prod-app-identity-decision-pending.md)의 후보 값을 확정값으로 사용하지 않는다. 웹/API·DB·프록시 준비는 계속 진행할 수 있다.
...
### 10.6 데스크톱 앱 이름과 기존 개발판 보존

2026-09-07 사용자는 운영판 이름(`Clipper` 또는 `Clipper Studio`)과 기존 아이콘 재사용 여부를 결정권자에게 문의했고 답변을 기다리는 중이다. 기존 개발판 설정·데이터·QA를 최대한 유지하며 운영판이 별도 저장소와 로그인·업데이트 연결을 갖도록 하는 것이 우선이다.

기존 개발판 데이터 이전이나 `clipper-dev://` 전환을 확정한 것은 아니다. 개발판의 `clipper://`를 유지하고 운영판에 `clipperstudio://`를 쓰는 안을 우선 검토하되, 구체적인 설정은 [결정 대기 기록](./2026-09-07-desktop-dev-prod-app-identity-decision-pending.md)에서 최종 결정 후 이어간다. 이름 논의 때문에 서버 준비를 중단하지 않으며, 운영 설치파일 공개 전에는 앱 공존과 환경 분리를 검증해야 한다.
