...
2026-05-29 갱신: 현재 `/jobs` 실행 boundary는 NestJS `WorkflowExecutorRegistry` 기준으로 확장됐다. `Compute Worker`는 여전히 중요한 축이지만, 모든 workflow executor가 Python/FastAPI process일 필요는 없다. NestJS-native executor도 `runtimeKind=nestjs_executor`로 같은 Plugin Store/job history/start-stop 모델에 들어올 수 있다.
...
- `WorkflowExecutor`: 특정 plugin/workflow job을 실행하는 NestJS dispatch 단위. Python runtime plugin, NestJS-native executor, virtual workflow를 구분한다.
...
- `Compute Worker`: 주로 FastAPI/Python plugin이 담당하는 model/AI/video 처리 계층. 단순 ffmpeg workflow처럼 NestJS-native executor로 충분한 경우도 있다.
...
Angular
  -> NestJS Control Plane
      ...
      -> NestJS-native Executors
          -> direct service/child_process execution
```
