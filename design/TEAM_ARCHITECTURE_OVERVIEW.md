...
작성 기준: 2026-05-26 현재 checkout 확인 결과.

이 문서는 팀원에게 Clipper2의 전체 구조를 설명하기 위한 문서다. 구현 세부사항보다 "어떤 repo가 어떤 책임을 갖고, 실행 시 어떤 프로세스들이 어떻게 연결되는지"를 이해하는 데 초점을 둔다.

## 한 문장 설명

Clipper2는 Angular UI, NestJS control plane, Electron desktop host, Python worker/plugin runtime이 분리된 다중 프로세스 데스크톱 앱이다.
...
핵심은 Angular가 모든 것을 직접 알지 않는다는 점이다. Angular는 NestJS API만 호출하고, NestJS가 workflow와 plugin 실행을 조율한다. Electron은 데스크톱 앱을 실제로 띄우고 OS/native 기능과 packaged process 실행을 담당한다. Python은 영상 분석, 렌더링, 모델 기반 처리처럼 무거운 작업을 수행한다.
...
- local/devapp에서는 Python plugin process를 직접 실행할 수 있다.
...
- packaged 앱의 native process host 역할을 직접 맡지 않는다.

팀원에게는 "`clipper_nestjs`는 Clipper2의 관제탑이다. UI 요청을 받아 workflow를 결정하고, 필요한 worker를 실행시키고, 결과를 다시 UI에 돌려준다"라고 설명하면 된다.
...
팀원에게는 "`clipper_electron`은 설치형 앱 껍데기이자 OS 연결 계층이다. packaged 모드에서는 내부 NestJS와 Python worker를 실제 프로세스로 띄우는 host 역할도 한다"라고 설명하면 된다.
...
4. NestJS는 필요한 plugin runtime을 찾거나 실행한다.
5. Python plugin이 실제 작업을 수행한다.
...
7. Python plugin이 필요하면 NestJS가 Electron plugin host bridge에 요청한다.
...
Angular Template Builder UI
  ...
  -> 필요 시 clipper1_video_render Python plugin
```
...
2026-05-26 세션에서 확인한 상태:

```text
.codex:           main                       @ f711942 Finalize codex refactor handoff
...
모든 repo는 확인 시점에 원격 추적 브랜치 기준 clean 상태였다.
...
3. NestJS는 control plane이다. 프로젝트, 템플릿, 작업 큐, plugin orchestration, image search 같은 앱의 중심 로직을 관리한다.
4. Electron은 데스크톱 host다. 창을 띄우고, packaged 앱에서 NestJS와 Python process를 실행하고, native IPC와 installer packaging을 담당한다.
5. Python은 worker/plugin runtime이다. 영상 분석, 렌더링, 모델 기반 처리처럼 무거운 작업을 한다.
...
mode에 따라 다르다. local/devapp에서는 NestJS가 Python plugin process를 직접 실행할 수 있다. packaged에서는 Electron이 native process host 역할을 하고, NestJS는 Electron bridge를 통해 제어한다.
